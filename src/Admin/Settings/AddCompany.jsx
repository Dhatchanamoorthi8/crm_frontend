...
const AddCompany = () => {
  ...
  const apiCall = async () => {
    try {
      const response = await api.get('companymaster')
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await api.post('companymaster', NewCompanyName)
      ...
      if (response.status === 201) {
        ...
        setAlertProps({
          alertType: 'Success',
          content: `Your New ${NewCompanyName.CompanyName} Name successfully!`,
          renderType: 'toast',
          visible: true
        })
  ...
  const handleEditConfirm = async () => {
    try {
      const response = await api.patch('companymaster', companyEditData)
      if (response.status === 200) {
        apiCall()
        SetModelState(c => ({ ...c, isOpen: false, type: '' }))
        setAlertProps({
          alertType: 'Success',
          content: `${companyEditData.CompanyName} Name Edited successfully!`,
          renderType: 'toast',
          visible: true
        })
      }
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setAlertProps({
          alertType: 'Success',
          content: `${companyEditData.CompanyName} Name Deleted successfully!`,
          renderType: 'toast',
          visible: true
        })
      }
      ...
      <View style={styles.cardWrapper} mb='$2'>
        {CompanyData && CompanyData.length > 0 ? (
          CompanyData.map((item, index) => (
            <Card variant='elevated' style={styles.card} key={item.cm_id}>
              <View
                display='flex'
                flexDirection='row'
                ...
                <FormControl p={'$2'} rounded={'$lg'}>
                  <VStack space='xl'>
                    <VStack space='xs'>
                      ...
                      >
                        <InputField
                          defaultValue={companyEditData.CompanyName}
                          value={companyEditData.CompanyName}
                          onChangeText={e =>
                            setcompanyEditData(current => ({
                              ...current,
                              CompanyName: e
